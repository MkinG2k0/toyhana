...
import type { BookingCard as BookingCardType } from "@/types/booking"
...
export const BookingCard = ({ booking, className }: BookingCardProps) => {
  const eventTypeLabel = EVENT_TYPE_LABELS[booking.eventType] ?? booking.eventType
  ...
  return (
    <Card className={className}>
      ...
      <CardContent className="space-y-3">
        ...
        {booking.message && (
          <p className="text-sm text-muted-foreground border-l-2 border-surface-300 pl-3">
            {booking.message}
          </p>
        )}
      </CardContent>
